function [e, h] = laguerreGaussFun( obj, pt, k0 )
% LAGUERREGAUSSFUN - Electromagnetic fields for a laguerre gauss beam
% 
% Input
...
if any( pt.tau( 1 ).inout == obj.imat )
  ...
  z = pos(:, :, 3);
  r = sqrt(x^2 + y^2);
  e(:, 1) = A * u_0(r, z) * u_m(r, z, p) * exp(1i*k1*z);
  e(:, 3) = ((m*(1i*y+x)/(k1*r.^2)) - ((1i*x)/(1i*z-zR)) ...
    - ((4i*x)/(k1*w(z).^2))) * e(:, 1);

  h(:, 2) = A * u_0(r, z) * u_m(r, z, p) * exp(1i*k1*z);
